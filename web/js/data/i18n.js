...
const MESSAGES = {
  zh: {
    ...
    'right.pick_title': '抓取',
    'right.cable_title': '缆绳流程',
    'right.cable_hint': '缆绳仅抓取，不做插孔。',
    'right.targetsensor_title': 'TargetSensor 流程',
    'right.targetsensor_hint': 'TargetSensor：先抓取，再按前端选择孔位执行插孔流程。',
    'right.target_index_label': '目标',
    'right.target_index_prefix': '目标',
    'right.target_slot_label': '孔位',
    'right.target_slot_prefix': '孔位',
    'right.target_insert': '执行插孔',
    'right.pick_cable': '缆绳抓取',
    'right.pick_cable_title': 'perception_state.cable_object_pose → submit_job LEGACY（与缆绳桥接一致）',
    'right.pick_target_sensor': 'TargetSensor 抓取',
    'right.pick_target_sensor_title': '按前端目标索引从 target_set 取位姿 → submit_job TARGET_SENSOR',
    'right.pick_fused': '视觉+声呐中心线',
    'right.pick_fused_title': '视觉 + 声呐 Keypoints → 中心线拟合 → /manipulator/object_pose_fused',
    'right.pick_hint': '缆绳 / TargetSensor 分两路提交 LEGACY；中心线为 FUSED。均需对应感知有有效位姿，submit_job 异步入队。',
    ...
    'viewport.joints': '关节角度 (Orion /joint_states)',
    'viewport.holoocean_joints': 'HoloOcean ArmSensor (°)',
    'viewport.holoocean_joint_col': 'left_arm',
    'viewport.holoocean_gripped': 'gripped',
    ...
    'card.perception.source_tab_cable': '缆绳',
    'card.perception.source_tab_target': 'TargetSensor',
    'card.perception.source_tab_aria': '感知源分栏',
    'card.perception.tab_has_data': '当前有有效位姿',
    'card.perception.cable_label': '缆绳（CableSensor）',
    'card.perception.cable_hint': 'perception_state · cable_object_pose · arm_base_link',
    'card.perception.target_sensor_label': '目标物（TargetSensor）',
    'card.perception.target_sensor_hint': 'perception_state · target_sensor_object_pose · arm_base_link',
    'card.perception.source_missing': '暂无数据（对应桥接未发布或位姿无效）',
    'card.perception.pos': '位置 (m)',
    'card.perception.quat': '姿态 (四元数)',
    'card.perception.updated': '更新时间',
    'card.perception.fused_label': '视觉+声呐 · 中心线抓取点',
    'card.perception.fused_hint': '/object_pose_fused · Keypoints · arm_base_link · ',
    ...
    'card.perception.kp_missing': '未收到 /manipulator/keypoints_arm_base_link（需 keypoint_to_arm_tf）',
    'card.perception.legend': '图例',
    'card.perception.legend_text': '青=/object_pose 抓取点 · 橙=TargetSensor · 灰柱=缆绳模型 · 品红=拟合 · 琥珀=关键点',
    'card.perception.rov_map': 'ROV位姿 (map)',
    'card.perception.rov_base': 'ROV位姿 (arm_base_link)',
    'card.perception.pose_orient': '姿态',
    'card.perception.table_idx': '#',
    'card.perception.table_pos': '位置 (m)',
    'card.perception.table_quat': '姿态(简)',
    ...
    'job.reset_held': '重置持物',
    'job.sync_held': '同步持物',
    'job.target_insert': 'TargetSensor 插孔',
    ...
    'toast.no_pose_cable': '无缆绳位姿（perception_state.cable_object_pose），无法提交缆绳抓取',
    'toast.no_pose_target_sensor': '无 TargetSensor 位姿（perception_state.target_sensor_object_pose），无法提交',
    'toast.no_pose_fused': '无中心线融合位姿，无法提交「视觉+声呐中心线」',
    'toast.pick_submit_fail': '抓取提交失败',
    'toast.pick_cable_ok': '缆绳抓取已提交',
    'toast.pick_target_sensor_ok': 'TargetSensor 抓取已提交',
    'toast.pick_fused_ok': '「视觉+声呐中心线」已提交',
    'toast.target_insert_ok': 'TargetSensor 插孔已提交',
    'toast.target_insert_fail': 'TargetSensor 插孔提交失败',
  ...
  en: {
    ...
    'right.pick_title': 'Pick',
    'right.cable_title': 'Cable flow',
    'right.cable_hint': 'Cable only picks; no insert.',
    'right.targetsensor_title': 'TargetSensor flow',
    'right.targetsensor_hint': 'TargetSensor: pick first, then insert using selected slot.',
    'right.target_index_label': 'Target',
    'right.target_index_prefix': 'Target ',
    'right.target_slot_label': 'Slot',
    'right.target_slot_prefix': 'Slot ',
    'right.target_insert': 'Run insert',
    'right.pick_cable': 'Pick (cable)',
    'right.pick_cable_title': 'perception_state.cable_object_pose → submit_job LEGACY',
    'right.pick_target_sensor': 'Pick (TargetSensor)',
    'right.pick_target_sensor_title': 'Use selected target index from target_set → submit_job TARGET_SENSOR',
    'right.pick_fused': 'Vision + sonar centerline',
    'right.pick_fused_title': 'Keypoints → centerline → /manipulator/object_pose_fused',
    'right.pick_hint': 'Cable vs TargetSensor: two LEGACY submit paths; centerline is FUSED. Requires valid poses; submit_job is async.',
    ...
    'right.ws_title': 'arm_base_link: indicative AABB after feasibility clamp (wireframe in 3D); corners may be unreachable',
    'right.ws_note': 'Stay inside',
    'right.target_cable': 'Target (cable)',
    'right.target_cable_title': 'Cable center in arm_base_link; same limits as check_pick / grasp (feasibility)',
    ...
    'viewport.joints': 'Joint angles (Orion /joint_states)',
    'viewport.holoocean_joints': 'HoloOcean ArmSensor (°)',
    'viewport.holoocean_joint_col': 'left_arm',
    'viewport.holoocean_gripped': 'gripped',
    ...
    'card.perception.source_tab_cable': 'Cable',
    'card.perception.source_tab_target': 'TargetSensor',
    'card.perception.source_tab_aria': 'Perception source tabs',
    'card.perception.tab_has_data': 'Valid pose',
    'card.perception.cable_label': 'Cable (CableSensor)',
    'card.perception.cable_hint': 'perception_state · cable_object_pose · arm_base_link',
    'card.perception.target_sensor_label': 'Target (TargetSensor)',
    'card.perception.target_sensor_hint': 'perception_state · target_sensor_object_pose · arm_base_link',
    'card.perception.source_missing': 'No data (bridge off or invalid pose)',
    ...
    'card.perception.kp_missing': 'No /manipulator/keypoints_arm_base_link (needs keypoint_to_arm_tf)',
    'card.perception.legend': 'Legend',
    'card.perception.legend_text': 'Cyan=/object_pose · Orange=TargetSensor · Gray=cable mesh · Magenta=fused · Amber=keypoints',
    'card.perception.rov_map': 'ROV (map)',
    'card.perception.rov_base': 'ROV (arm_base_link)',
    'card.perception.pose_orient': 'Orient',
    'card.perception.table_idx': '#',
    'card.perception.table_pos': 'Pos (m)',
    'card.perception.table_quat': 'Quat',
    'card.perception.table_id': 'ID',
    ...
    'job.target_insert': 'TargetSensor insert',
    ...
    'toast.no_pose_cable': 'No cable pose (perception_state.cable_object_pose)',
    'toast.no_pose_target_sensor': 'No TargetSensor pose (perception_state.target_sensor_object_pose)',
    'toast.no_pose_fused': 'No fused pose for centerline pick',
    'toast.pick_submit_fail': 'Pick submit failed',
    'toast.pick_cable_ok': 'Cable pick submitted',
    'toast.pick_target_sensor_ok': 'TargetSensor pick submitted',
    'toast.pick_fused_ok': 'Centerline pick submitted',
    'toast.target_insert_ok': 'TargetSensor insert submitted',
    'toast.target_insert_fail': 'TargetSensor insert submit failed',
