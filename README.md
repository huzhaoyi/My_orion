...
| **orion_mtc** | 基于 MoveIt Task Constructor 的抓放节点；任务队列 + 后台 Worker 消费，支持优先级与去重；**抓取**：优先使用 `/object_pose` 话题位姿，预抓取竖直朝下、approach 沿 base_link -Z 竖直下压（无旋转）；夹取失败时自动回到 ready 并设 IDLE；**放置**：PlaceGenerator 多候选依次尝试，完成后回到 ready（臂+手张开）；另提供 **打开/闭合夹爪** 服务（仅动夹爪，臂关节保持当前 joint_states）；Action 即时执行，话题/SubmitJob 异步入队 |
| **orion_holoocean_bridge** | HoloOcean 桥接：ArmSensor → `joint_states`，轨迹 → HoloOcean Agent；TargetSensor + ROV 里程计 → `/object_pose`（base_link） |
...
- **抓取**：使用 `/manipulator/object_pose` 话题位姿（base_link）；预抓取竖直朝下，approach 沿 base_link -Z 竖直下压；夹不到时判失败并自动回到 ready（臂+手张开）再设 IDLE。抓稳后保存持物上下文，状态变为 HOLDING_TRACKED/HOLDING_UNTRACKED。
...
| 话题 | `/manipulator/object_pose` | 物体位姿（PoseStamped，base_link）；抓取使用此位姿，由桥接或外部发布 |
...
桥接 `target_sensor_to_object_pose` 从 TargetSensor + ROV 里程计**自动发布** `/manipulator/object_pose`（base_link）。流程为**拆分接口**：先抓取再放置（或 place_release），无一体化 pick-place 话题。所有接口均在 `/manipulator` 命名空间下。**TF**：本 launch 下机械臂相关 TF 发布/订阅在 `/manipulator/tf`、`/manipulator/tf_static`（由 `tf_under_manipulator:=true` 启用），与仿真侧或其他 TF 源隔离。
...
4. **业务规则**：未持物时 place 被拒绝；已持物时 pick 被拒绝。队列满或策略 `reject_new_jobs_while_busy` 时 submit_job 返回拒绝原因。抓取使用 object_pose 位姿；放置对单次 target_pose 会内部多候选尝试。夹取失败时自动回到 ready 再设 IDLE。
