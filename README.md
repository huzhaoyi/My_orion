...
| **orion_mtc** | 基于 MoveIt Task Constructor 的抓放节点；任务队列 + 后台 Worker 消费，支持优先级与去重；**抓取**：使用 `/object_pose`（position + orientation）；姿态由 **direction（圆柱轴）** 推导为**侧向抓取**（闭合方向与接近方向均垂直于圆柱轴），pregrasp 沿接近方向偏移、approach 沿该方向接近物体；话题触发的 PICK 执行前会取话题最新位姿；夹爪相对 Link6 的 Z 偏移通过 `gripper_tip_offset_from_link6_z` 补偿；夹取失败时自动回到 ready 并设 IDLE；**放置**：PlaceGenerator 多候选依次尝试，放置高度同样做夹爪偏移补偿；另提供 **打开/闭合夹爪** 服务（仅动夹爪，臂关节保持当前 joint_states）；Action 即时执行，话题/SubmitJob 异步入队 |
| **orion_holoocean_bridge** | HoloOcean 桥接：ArmSensor → `joint_states`；FollowJointTrajectory 经 **trajectory_to_agent_bridge** 转为 AgentCommand 发往 HoloOcean 执行；**target_sensor_to_object_pose**：TargetSensor（positions + directions）+ ROV 里程计 → base_link 下 `/object_pose`（位置=目标中心，姿态=由 direction 构造的侧向抓取系）及 TargetSet；**cable_sensor_to_object_pose**：CableSensor（单缆绳）→ world 下变换到 base_link 后发布 `/object_pose` 与 `/place_pose`（缆绳单目标，供 MTC 抓取/放置） |
...
- **抓取**：使用 `/manipulator/object_pose`（base_link，position + orientation）。姿态由 **direction（圆柱轴）** 推导为**侧向抓取**（夹爪闭合方向与接近方向均垂直于圆柱轴）；pregrasp 位于物体中心沿接近方向偏移，approach 沿该方向接近。话题触发的 PICK 在执行时会取话题**最新**位姿（先等待一次更新再取 latest）。夹不到时判失败并自动回到 ready（臂+手张开）再设 IDLE。抓稳后保存持物上下文，状态变为 HOLDING_TRACKED/HOLDING_UNTRACKED。
- **放置**：仅在 HOLDING 时可用；输入为物体目标位姿，PlaceGenerator 在目标位附近采样多候选，依次规划/执行；成功后 pre-place → lower → open → detach → retreat，再 **回到 ready**（臂+手张开）、清空持物、IDLE。
- **放置释放**（PlaceRelease）：按 TCP 目标位姿执行放置并 detach，完成后同样回到 ready。
- **打开/闭合夹爪**：仅对手 group 做 MoveTo open/close，**臂关节保持当前 joint_states**（来自 joystick/编码器等），不入队则即时通过服务触发并异步入队执行。
...
| 服务 | `/manipulator/submit_job` | 异步入队：提交 PICK/PLACE/PLACE_RELEASE/RESET_HELD_OBJECT/SYNC_HELD_OBJECT/**OPEN_GRIPPER**/**CLOSE_GRIPPER**，返回 `job_id` 或拒绝原因 |
...
| 话题 | `/manipulator/object_pose` | 物体位姿（PoseStamped，base_link）：位置=目标中心，姿态=侧向抓取系（由 direction 构造，y=闭合方向、z=接近方向）；抓取使用此位姿，桥接由 TargetSensor 实时发布，执行抓取时取最新一帧 |
| 话题 | `/manipulator/place_pose` | 放置目标位姿（PoseStamped），放置使用；内部会生成多候选依次尝试 |
| 话题 | `/manipulator/pick_trigger` | 空消息：异步入队**抓取**（需有 object_pose，无则等待 3s 或拒绝） |
...
业务规则：未持物时禁止 place；已持物时禁止再次 pick；**夹爪 locked（有物，来自 /left_arm_gripped）时禁止 pick**，避免 object 在夹爪上导致规划失败，需先 place 或 open_gripper。队列默认按优先级调度（RESET/SYNC/PLACE_RELEASE 高于 PICK/PLACE），同类型同目标短时间窗口内去重；配置见 `orion_mtc/config/runtime_policy.yaml`、`pick_place_params.yaml`。
...
桥接提供两种目标源（launch 中均会启动）：**target_sensor_to_object_pose** 从 TargetSensor（positions + directions）+ ROV 里程计变换到 base_link 并发布 `/manipulator/object_pose`；**cable_sensor_to_object_pose** 从 CableSensor（单缆绳）变换到 base_link 并发布 `/manipulator/object_pose` 与 `/manipulator/place_pose`。位姿为**侧向抓取系**（由 direction/圆柱轴构造：y=闭合方向、z=接近方向）。目标会随传感器实时更新；话题触发的抓取在执行前会取最新位姿。流程为**拆分接口**：先抓取再放置（或 place_release），无一体化 pick-place 话题。所有接口均在 `/manipulator` 命名空间下。**TF**：本 launch 下机械臂相关 TF 发布/订阅在 `/manipulator/tf`、`/manipulator/tf_static`（由 `tf_under_manipulator:=true` 启用），与仿真侧或其他 TF 源隔离。

**前置条件（抓取）**：需有 `/manipulator/object_pose`（或等待 3s），否则 pick_trigger 不入队。若 HoloOcean 未跑或暂无目标，可：① 先启动 HoloOcean 并确保有目标；或 ② 手动发布 object_pose 再 pick_trigger；或 ③ 用 Action `/manipulator/pick` 并在 Goal 里带 `object_pose`。
...
- **工作空间限值**（水平/垂直 min～max，防止抓取/放置目标配置超出）：见 [docs/workspace_limits.md](docs/workspace_limits.md)；**TF 与坐标系**（world/ROV/base_link）：见 [docs/tf_conversion.md](docs/tf_conversion.md)
...
**前置**：先启动 rosbridge（如 `ros2 launch rosbridge_server rosbridge_websocket_launch.xml`）和 orion_mtc（如 `pick_place_holoocean.launch.py`）。
...
**功能对应**：
- **订阅话题**：`runtime_status`、`job_event`、`task_stage`、`held_object_state`、`recovery_event`、`object_pose`、`place_pose`、`joint_states`
- **调用服务**：`get_robot_state`（连接时同步）、`get_queue_state`、`get_recent_jobs`（底部「最近执行」Tab）、`submit_job`、`cancel_job`、`reset_held_object`、`sync_held_object`、`open_gripper`、`close_gripper`
- **界面**：左侧当前执行/队列/持物/感知/最近错误，中间 3D 视图与视角/显示层，右侧任务操作（Pick/Place/PlaceRelease）与调试工具，底部事件流/最近执行/系统日志
...
3. **终端 2：触发与状态查询**
   ...
   - **抓取**（需先有 object_pose）：`ros2 topic pub --once /manipulator/pick_trigger std_msgs/msg/Empty "{}"`；若 3s 内无 object_pose 则不入队。
...
4. **业务规则**：未持物时 place 被拒绝；已持物时 pick 被拒绝。队列满或策略 `reject_new_jobs_while_busy` 时 submit_job 返回拒绝原因。抓取使用 object_pose（侧向抓取姿态由 direction 推导，话题触发时执行前取最新位姿）；放置对单次 target_pose 会内部多候选尝试，放置高度做夹爪偏移补偿。夹取失败时自动回到 ready 再设 IDLE。
