...
std::string resolve_target_stl_path()
{
  ...
  const std::string primary = share + "/meshes/stl/target_square.stl";
  std::ifstream check_primary(primary);
  if (check_primary.good())
  {
    return primary;
  }
  const std::string legacy = share + "/target_square.stl";
...
/*
 * TargetSensor peg：嵌入 STL 三角网格 + mesh_pose（与网页 target_square.stl 对齐/躺平一致）；无 STL 时退化为定向盒。
 */
