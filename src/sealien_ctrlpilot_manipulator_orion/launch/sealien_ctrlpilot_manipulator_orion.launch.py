#!/usr/bin/env python3
"""
Orion 机械臂抓取（MTC）与 HoloOcean 联调：关节状态来自 /holoocean/rov0/ArmSensor（right_arm 6DOF+夹爪）。
...
MTC 执行：sealien_ctrlpilot_manipulator_orion_mtc_node 将规划得到的轨迹发送到 arm_controller / hand_controller 的
...
是否默认启动手柄桥接 / RViz：见本包 config/default.yaml（launch 参数仍可覆盖）。
孔位旁面板 panel_obstacles：见 sealien_ctrlpilot_manipulator_orion_holoocean_bridge/config/panel_obstacles_mtc_fragment.yaml；
本 launch 合并 panel_obstacles 进 sealien_ctrlpilot_manipulator_orion_mtc_node，并写入 target_sensor_to_object_pose 参数，使面板 Marker 与 /manipulator/target_insert_holes 同 stamp 发布。
"""
import copy
import os
import tempfile
...
_LAUNCH_ONLY_PARAM_KEYS = frozenset({
    "use_joy_manipulator",
    "start_rviz",
...
def _append_rosbridge_after_stack(context, *_args, **_kwargs):
    ...
    share = get_package_share_directory("sealien_ctrlpilot_manipulator_orion_mtc")
...
def generate_launch_description():
    ...
    从本包 config/default.yaml 读取 launch 默认；MTC 节点参数来自 sealien_ctrlpilot_manipulator_orion_mtc_params.yaml。
    """
    manipulator_share = get_package_share_directory("sealien_ctrlpilot_manipulator_orion")
    launch_defaults_path = os.path.join(manipulator_share, "config", "default.yaml")
    launch_cfg = {}
    if os.path.isfile(launch_defaults_path):
        with open(launch_defaults_path, "r") as f:
            loaded_launch = yaml.safe_load(f)
        if isinstance(loaded_launch, dict):
            launch_cfg = loaded_launch
    ...
    orion_holoocean_share = get_package_share_directory("sealien_ctrlpilot_manipulator_orion_holoocean_bridge")
    sealien_ctrlpilot_manipulator_orion_mtc_share = get_package_share_directory("sealien_ctrlpilot_manipulator_orion_mtc")
    orion_joy_share = get_package_share_directory("sealien_ctrlpilot_manipulator_orion_joy_arm_bridge")
    ...
    mtc_app_params_path = os.path.join(sealien_ctrlpilot_manipulator_orion_mtc_share, "config", "sealien_ctrlpilot_manipulator_orion_mtc_params.yaml")
    mtc_app_raw = {}
    ...
    use_joy_default = _yaml_bool_to_launch_arg_string(launch_cfg.get("use_joy_manipulator", False))
    start_rviz_default = _yaml_bool_to_launch_arg_string(launch_cfg.get("start_rviz", False))
    rosbridge_delay_default = str(launch_cfg.get("rosbridge_startup_delay_sec", 5.0))
    rosbridge_port_default = str(launch_cfg.get("rosbridge_port", 9091))
    rosbridge_address_default = str(launch_cfg.get("rosbridge_address", "0.0.0.0"))
    ...
    mtc_node = Node(
        ...
        executable="mtc_node",
        name="sealien_ctrlpilot_manipulator_orion_mtc_node",
        output="screen",
    ...
    arg_use_joy = DeclareLaunchArgument(
        ...
        description=(
            "true：启动 sealien_ctrlpilot_manipulator_orion_joy_arm_bridge；默认取自 share/sealien_ctrlpilot_manipulator_orion/config/default.yaml"
        ),
    )
    arg_start_rviz = DeclareLaunchArgument(
        ...
        description=(
            "true：启动 RViz2；默认取自 share/sealien_ctrlpilot_manipulator_orion/config/default.yaml"
        ),
    )

    keypoint_launch = os.path.join(sealien_ctrlpilot_manipulator_orion_mtc_share, "launch", "keypoint_arm_tf.launch.py")
    ...
    arg_rosbridge_delay = DeclareLaunchArgument(
        "rosbridge_startup_delay_sec",
        default_value=rosbridge_delay_default,
        description=(
