...
{
  /* 预抓沿接近方向 n 相对闭合准备位的距离；空则回退 approach_dist_candidates */
  ...
  /* gripper_tcp 相对抓取语义系（y=缆绳轴、z=接近）的固定旋转：ZYX 外旋，单位 ° */
  ...
  /* 缆绳 pick「approach to grasp (LIN)」Pilz 缩放；上限见 joint_limits max_acceleration，默认 1.0 易超限 */
...
struct MTCConfig
{
  /* pick lift（缆绳 pick 的抬升段见 pick_task_builder / cable_side_grasp） */
